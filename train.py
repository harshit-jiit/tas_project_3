...
from modernized_loader.tas_balanced_training_loader import TASBalancedDatasetLoader 
import numpy
import random
import transformers
import signal
...
def timeout_handler(signum, frame):
    raise TimeoutException("Batch processing timeout")

# Add this function at the top level
def log_memory_and_disk():
    """Log memory and disk usage"""
    try:
        # GPU memory
        ...
        print(f"GPU Mem: {gpu_mem:.2f}GB, GPU Cache: {gpu_cache:.2f}GB, Disk Free: {free_gb}GB")
        logger.info(f"GPU Mem: {gpu_mem:.2f}GB, GPU Cache: {gpu_cache:.2f}GB, Disk Free: {free_gb}GB")
        
        return free_gb > 1  # Return False if critically low on space
    except:
        return True
...
def check_disk_space_and_adjust(config, current_workers=None):
    """Monitor disk space and adjust workers dynamically"""
    try:
        total, used, free = shutil.disk_usage("/")
        free_gb = free // (2**30)
        
        if free_gb < 1:  # Critical - go to 0 workers
            return 0
        elif free_gb < 2:  # Low - reduce workers
            return max(1, current_workers - 1) if current_workers else 1
        elif free_gb > 5:  # Good space - can use more workers
            return min(4, current_workers + 1) if current_workers else 2
        else:
            return current_workers if current_workers else 2
...
def aggressive_cleanup():
    ...
    try:
        ...
        # Clean system temp
        try:
            subprocess.run(['find', '/tmp', '-name', 'torch_*', '-delete'], 
                          stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, timeout=30)
            subprocess.run(['find', '/tmp', '-name', 'pymp-*', '-delete'], 
                          stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, timeout=30)
        except:
            pass
...
if __name__ == "__main__":
    # Set custom temp directory with more space - ADD THIS FIRST!
    # Call this right after the main config setup (around line 200):
    # Call cleanup before training starts  
    aggressive_cleanup()
    cleanup_temp_files()
    
    base_path = "/workspace/2404170001/"
    #
    # config - replaced args parser and YAML config with direct config dictionaries
    #
    ...
    model_config={
        ...
        "validate_every_n_batches": 4000, #4000,
        ...
        "param_group1_learning_rate": 0.000007,
        "param_group1_weight_decay": 0.01,
        "embedding_optimizer": "adam",
        "embedding_learning_rate": 0.000007,
        "embedding_weight_decay": 0.01,
        "embedding_optimizer_learning_rate": 0.000007,
        "embedding_optimizer_momentum": 0.8,
        "use_title_body_sep": False,
        "use_cls_scoring": False,
        "minimize_sparsity": False,
        "sparsity_log_path": "/path/to/sparsity/log",
        ...
        "min_steps_training": 300000,
        "validation_metric": "nDCG@10",
        "loss": "margin-mse",
        "in_batch_neg_loss": "margin-mse",
        "epochs": 25,
        "dynamic_sampler": True,
        "dynamic_sampler_type": "tas_balanced",
        "tas_balanced_margin_bins": 10,
        "dynamic_query_file": os.path.join(base_path,"inputs/queries.train.tsv"),
        "dynamic_collection_file": os.path.join(base_path,"inputs/collection.tsv"),
        "dynamic_pairs_with_teacher_scores": os.path.join(base_path,"inputs/T2-train-ids.tsv"),
        "dynamic_query_cluster_file": os.path.join(base_path,"clustering_output1/cluster-assignment-ids.tsv"),
        ...
        "min_doc_length": -1,
        "min_query_length": -1,
        "tas_balanced_pair_strategy": "bins",
        "tokenizer_name": "distilbert-base-uncased",
        "dynamic_teacher_path": os.path.join(base_path,"tas_project_3/config/huggingface_modelhub/sebastian-hofstaetter/config.yaml"),
        "dynamic_teacher_in_batch_scoring": True,
        "dynamic_teacher_per_term_scores": False,
        "token_embedder_type": "bert_dot",
        "gradient_accumulation_steps": -1,
        "in_batch_main_pair_lambda": 1,  # Weight for main pairwise loss
        "in_batch_neg_lambda": 0.75,
        "qa_loss_lambda": 0.2,
        "sparsity_loss_lambda_factor": 0.01,
        "sparsity_reanimate_lambda": False,
        "sparsity_reanimate_bias": 0.5,
        "use_title_body_sep": False,
        "use_cls_scoring": False,
        "minimize_sparsity": False,
        # "submodel_validation_cache_path": "/path/to/submodel/validation/cache",
        "query_augment_mask_number": -1,
        "batch_size_eval": 512,
        "dataloader_num_workers": 0,
        "validation_cont_use_cache": True
    }
    print("Using train_config:", train_config)
    print("Using model_config:", model_config)

    # Create a simple args-like object to maintain compatibility with existing functions
    class Args:
    ...
    if args.continue_folder:
        ...
        # Load existing config from the continue folder if needed
        # For now, we'll use our model_config as the base
        config = model_config.copy()
    else:
        if not args.run_name:
            raise Exception("--run-name must be set (or continue-folder)")
        # Use our direct config dictionary instead of loading from YAML
        config = model_config.copy()
        run_folder = prepare_experiment(args, config)

    logger = get_logger_to_file(run_folder, "main")
    logger.info("Running with train_config: %s", str(train_config))
    logger.info("Running with model_config: %s", str(model_config))
    ...
    try:
        ...
        if from_scratch:
            for epoch in range(0, int(config["epochs"])):
                ...
                input_loader = TASBalancedDatasetLoader(
                    query_file=config["dynamic_query_file"],
                    collection_file=config["dynamic_collection_file"],
                    pairs_with_teacher_scores=config["dynamic_pairs_with_teacher_scores"],
                    query_cluster_file=config["dynamic_query_cluster_file"],
                    batch_size=config["batch_size_train"],
                    clusters_per_batch=config["dynamic_clusters_per_batch"],
                    tokenizer_name=config["tokenizer_name"],
                    max_doc_length=config["max_doc_length"],
                    max_query_length=config["max_query_length"],
                    pair_balancing_strategy=config["tas_balanced_pair_strategy"],
                    random_seed=config["random_seed"],
                )
                # input_loader = modern_tas_balanced_loader(config, config)
                if use_dynamic_teacher:
                    ...
                    for batch in input_loader:
                        if i == 0:
                            status.update("[bold magenta]           Starting...",refresh=True)
                                                # Clean temp files every 500 batches (not every batch!)
                                                # Add comprehensive logging every 50 batches
                        if i > 0 and i % validate_every_n_batches == 0:
                            status.update(f"[bold magenta]           Processing batch {i}...", refresh=True)
                            if not log_memory_and_disk():
                                logger.warning(f"Low disk space at batch {i}")
                                cleanup_temp_files()

                        if i > 0 and i % 2000 == 0:
                            cleanup_temp_files()
                        
                        # Add timeout for each batch (60 seconds)
                        # signal.signal(signal.SIGALRM, timeout_handler)
                        # signal.alarm(60)  # 60 second timeout per batch

                        try:
                            batch_start_time = time.time()
                            if i % 1000 == 0:  # Only log every 100 batches
                                logger.info(f"Starting batch {i}")
                            batch = move_to_device(batch, cuda_device)
                            # logger.info(f"Batch {i}: moved to device")
                            ...
                            # logger.info(f"Batch {i}: prepared inputs")

                            #
                            # run model forward
                            #
                            output_pos = model.forward(*pos_in, use_fp16 = use_fp16)
                            # logger.info(f"Batch {i}: forward pass pos done")
                            output_neg = model.forward(*neg_in, use_fp16 = use_fp16)
                            # logger.info(f"Batch {i}: forward pass neg done")
                            # signal.alarm(0)

                            batch_time = time.time() - batch_start_time
                            if batch_time > 30:  # Log slow batches
                                logger.warning(f"Slow batch {i}: {batch_time:.2f} seconds")
                            elif i % 1000 == 0:
                                logger.info(f"Batch {i} completed in {batch_time:.2f} seconds")
                        ...
                        #     signal.alarm(0)
                        #     logger.error(f"TIMEOUT: Batch {i} timed out after 60 seconds")
                        #     cleanup_temp_files()
                        #     torch.cuda.empty_cache()
                        #     log_memory_and_disk()
                            
                        #     # Skip this batch and continue
                        #     status.update(f"[bold red]           Batch {i} timed out - skipping...", refresh=True)
                        #     continue
