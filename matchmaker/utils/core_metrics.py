...
def calculate_metrics_along_candidate_depth(ranking, qrels, candidate_ranking, candidate_range, binarization_point=1.0):
    ...
    '''
    ranking = {str(qid): [str(doc_id) for doc_id in doc_list] for qid, doc_list in ranking.items()}
    qrels = {str(qid): {str(doc_id): rel for doc_id, rel in doc_dict.items()} for qid, doc_dict in qrels.items()}
    candidate_ranking = {str(qid): {str(doc_id): rank for doc_id, rank in doc_dict.items()} for qid, doc_dict in candidate_ranking.items()}
    
    print("TOP 10 entries in ranking:", list(ranking.items())[:10])
    print("TOP 10 entries in qrels:", list(qrels.items())[:10])
    print("TOP 10 entries in candidate_ranking:", list(candidate_ranking.items())[:10])
    ...
    for query_index,(query_id,ranked_doc_ids) in enumerate(ranking.items()):
        if query_id in qrels:
            ...
            # Filter documents first - before creating any arrays or masks
            candidates = candidate_ranking[query_id]
            candidate_positions = []
            filtered_ranked_doc_ids = []
            
            for d_id in ranked_doc_ids:
                d_id_str = str(d_id)
                if d_id_str in candidates:
                    candidate_positions.append(candidates[d_id_str])
                    filtered_ranked_doc_ids.append(d_id_str)
                else:
                    print(f"WARNING: Document {d_id_str} not in candidate set for query {query_id}, using max rank")
            
            # Update ranked_doc_ids to filtered version
            ranked_doc_ids = filtered_ranked_doc_ids
            candidate_positions = np.array(candidate_positions)
            
            # Now create all arrays from the filtered list
            np_rank = np.array(ranked_doc_ids)
            relevant_mask = np.in1d(np_rank, relevant_ids)
            binary_relevant_mask = np.in1d(np_rank, binary_relevant)
            
            # Apply consistent padding to all arrays to reach candidate_range[1]
            target_length = candidate_range[1]
            current_length = len(ranked_doc_ids)
            
            if current_length < target_length:
                pad_size = target_length - current_length
                relevant_mask = np.pad(relevant_mask, (0, pad_size), 'constant', constant_values=False)
                binary_relevant_mask = np.pad(binary_relevant_mask, (0, pad_size), 'constant', constant_values=False)
                np_rank = np.pad(np_rank, (0, pad_size), 'constant', constant_values='')
                candidate_positions = np.pad(candidate_positions, (0, pad_size), 'constant', constant_values=candidate_range[1]+2)
            elif current_length > target_length:
                # Truncate if longer than target
                relevant_mask = relevant_mask[:target_length]
                binary_relevant_mask = binary_relevant_mask[:target_length]
                np_rank = np_rank[:target_length]
                candidate_positions = candidate_positions[:target_length]
