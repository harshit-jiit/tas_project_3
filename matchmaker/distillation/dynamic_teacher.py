...
class DynamicTeacher():
    ...
    def __init__(self,
        ...
        self.base_path = "/workspace/2404170001/"
    ...
    def dynamic_teacher_subprocess(self, queue):
        ...
        try:
            ...
            console.log("[DynamicTeacher] Model config:",model_config)
            word_embedder, padding_idx = get_word_embedder(model_config)
            console.log("[DynamicTeacher] Word embedder:",word_embedder)
            model, encoder_type = get_model(model_config,word_embedder,padding_idx)
            console.log("[DynamicTeacher] Model:",model)
            ...
            #     model_path = cached_file(hf_bucket_url(self.dynamic_teacher_path, WEIGHTS_NAME))
            # else:
            #     model_path = os.path.join(self.dynamic_teacher_path, "best-model.pytorch-state-dict")
            model_path = os.path.join(self.base_path,"temp/colbert-distilbert-margin_mse-T2-msmarco/pytorch_model.bin")
            console.log("[DynamicTeacher] Model path:",model_path)
            load_result = model.load_state_dict(torch.load(model_path,map_location="cpu"),strict=False)
            console.log("[DynamicTeacher] Model load result:",load_result)
