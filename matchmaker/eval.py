...
def evaluate_model(model, config, _logger, cuda_device, eval_tsv, use_cache=False, 
    ...
    try:
        ...
        if not use_cache or fill_cache:
            ...
            input_loader = pytorch_reranking_inference_loader(config, config, eval_tsv)
                    ...
                    for sample_i, sample_query_id in enumerate(batch_orig["query_id"]):  # operate on cpu memory
                        ...
                        if train_qa_spans:
                            ...
                            if answerable[sample_i]:
                                ose=batch_orig["offsets_start"][sample_i][start_idx[sample_i]]
                                oe=batch_orig["offsets_end"][sample_i][end_idx[sample_i]]
                                if oe != None and ose != None and oe >= ose:
                                    qa_validation_results[sample_query_id][sample_doc_id] = (bool(answerable[sample_i]), int(start_idx[sample_i]), int(end_idx[sample_i]), batch_orig["doc_text"][sample_i][ose:oe])
                                else: 
...
def validate_model(val_name, model, config, validation_config, run_folder, _logger, cuda_device, 
    ...
    if candidate_set != None:
        # save the ranked_results in a file in json
        # save the validation result also in json
        with open(os.path.join(run_folder, "validation-output.json"), "w") as f:
            json.dump(validation_results, f)

        with open(os.path.join(run_folder, "validation-ranked-results.json"), "w") as f:
            json.dump(ranked_results, f)

        dd = load_qrels(validation_config["qrels"])
        with open(os.path.join(run_folder, "validation-qrels.json"), "w") as f:
            json.dump(dd, f)
        
        # candidate_Set
        with open(os.path.join(run_folder, "validation-candidate-set.json"), "w") as f:
            json.dump(candidate_set, f)

        metrics = calculate_metrics_along_candidate_depth(ranked_results, dd, candidate_set, validation_config["candidate_set_from_to"], validation_config["binarization_point"])
